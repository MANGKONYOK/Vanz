...
export default function FavStoresReportView({ showToast }) {
    ...
    return (
        <div className="fade-in space-y-5">
            ...
            <Card>
                {loading ? (
                    <div className="py-12 text-center text-slate-500 text-sm">Loading favorite stores…</div>
                ) : (
                    <Table headers={[{ label: 'Customer Name' }, { label: 'Favorite Store' }]}>
                        {filtered.length === 0 ? (
                            <tr><td colSpan={2} className="py-10 text-center text-slate-400 text-sm">No favorite store records found</td></tr>
                        ) : filtered.map((f, i) => (
                            <Tr key={i}>
