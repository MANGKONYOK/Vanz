...
export default function DelivererRankingReportView({ showToast }) {
    ...
    return (
        <div className="fade-in space-y-5">
            ...
            <Card>
                {loading ? (
                    <div className="py-12 text-center text-slate-500 dark:text-gray-300 text-sm">Loading deliverers…</div>
                ) : (
                    <Table headers={[
                        ...
                        {filtered.length === 0 ? (
                            <tr><td colSpan={5} className="py-10 text-center text-slate-500 dark:text-gray-300 text-sm">No deliverers found</td></tr>
                        ) : filtered.map((d, i) => (
                            <Tr key={d.id}>
                                <Td center><RankBadge rank={i + 1} /></Td>
                                <Td bold>{d.name}</Td>
                                <Td>{d.type}</Td>
                                <Td><span className="text-xs font-semibold text-slate-500 dark:text-gray-300">{d.status}</span></Td>
