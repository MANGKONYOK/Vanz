...
export default function UnapprovedVouchersReportView({ showToast }) {
    ...
    return (
        <div className="fade-in space-y-5">
            ...
            <Card>
                {loading ? (
                    <div className="py-12 text-center text-current/60 text-sm">Loading vouchers…</div>
                ) : (
                    <Table headers={[
                        ...
                        {unapproved.length === 0 ? (
                            <tr><td colSpan={6} className="py-10 text-center text-current/50 text-sm">No unapproved vouchers found</td></tr>
                        ) : unapproved.map(v => (
                            <Tr key={v.id}>
