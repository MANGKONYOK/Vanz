...
export default function StoreProductsReportView({ showToast }) {
    ...
    return (
        <div className="fade-in space-y-5">
            ...
            <Card>
                {loading ? (
                    <div className="py-12 text-center text-slate-500 dark:text-gray-300 text-sm">Loading products…</div>
                ) : (
                    <Table headers={[
                        ...
                        {filtered.length === 0 ? (
                            <tr><td colSpan={4} className="py-10 text-center text-slate-500 dark:text-gray-300 text-sm">No products found</td></tr>
                        ) : filtered.map(p => (
                            <Tr key={p.id}>
                                <Td>{p.store}</Td>
                                <Td bold>{p.name}</Td>
