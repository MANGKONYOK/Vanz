        ...
        try {
            if (isNew) {
                ...
                await Promise.all([
                    ...
                    putJson(`/customers/${data.customerCode}`, {
                        membership_level: membership,
                    }),
                ]);
        ...
        <div className="fade-in space-y-5">
            ...
            <Card className="p-5">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-current text-lg">{isNew ? 'New Customer' : `Edit: ${data.name}`}</h3>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <FormField label="Customer Code">
                        <Input value={isNew ? '(assigned on save)' : data.customerCode} readOnly
                    ...
                    <FormField label="Full Name" required>
                        <Input value={name} onChange={e => setName(e.target.value)} placeholder="Customer full name" />
                    </FormField>
