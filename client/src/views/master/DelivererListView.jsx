...
export default function DelivererListView({ showToast }) {
        ...
        Promise.all([
            ...
            setRows(deliverers.map(d => {
                ...
                return {
                    ...
                    rating:        d.rating != null ? parseFloat(d.rating).toFixed(1) : '0.0',
                };
            }));
        }).catch(err => {
    ...
    if (editing !== null) {
        return <DelivererFormView data={editing} onBack={() => setEditing(null)} onSaved={() => { setEditing(null); refresh(); }} showToast={showToast} />;
    }
            ...
            <Card className="overflow-hidden">
                ...
                <Table
                    ...
                    headers={[
                        ...
                        { label: 'RATING',  center: true, key: 'rating', sortable: true, width: '8%'  },
                        { label: 'Actions', right: true,                                  width: '14%' },
                    ]}
                >
                    {loading ? (
                        <Tr><Td colSpan={8} className="text-center text-current/40 py-8">Loading…</Td></Tr>
                    ) : paginated.length === 0 ? (
                        <Tr><Td colSpan={8} className="text-center text-current/40 py-8">No deliverers found</Td></Tr>
                    ) : paginated.map(d => (
                        <Tr key={d.delivererCode}>
                            <Td mono className="text-xs text-current/60 font-bold whitespace-nowrap">{d.delivererCode}</Td>
                            ...
                            <Td center className="whitespace-nowrap">
                                <span className="flex items-center justify-center gap-1">
                                    <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />{d.rating}
                                </span>
                            </Td>
                ...
                <Pagination totalItems={filtered.length} itemsPerPage={pageSize} currentPage={page} onPageChange={setPage} showSummary={false} itemLabel="deliverers" />
            </Card>
        </div>
    );
}
